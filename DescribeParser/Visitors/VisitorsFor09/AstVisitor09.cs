...
{
    /// <summary>
    /// An Ast visitor for Describe Language version 0.9
    /// Used to build Abstract Syntax Trees.
    /// </summary>
    public class AstVisitor09
    {
        /// <summary>
        /// Ctor.
        /// </summary>
        ...
        public AstScriptureNode TranslateScripture(Describe09Parser.ScriptureContext context, string filename = "")
        {
            ...
            {
                AstExpressionNode expr = DoExpression((child as Describe09Parser.ExpressionContext)!);
                expressions.Add(expr);
            }
            else if (child is Describe09Parser.Expression_listContext)
            {
                expressions = DoExpressionList((child as Describe09Parser.Expression_listContext)!);
            }
            else
            {
        ...
        /// </summary>
        private List<AstExpressionNode> DoExpressionList(Describe09Parser.Expression_listContext context)
        {
        ...
        /// </summary>
        private AstExpressionNode DoExpression(Describe09Parser.ExpressionContext context)
        {
            // sanity check
            int childCount = context.ChildCount;
            ...
            // get arrow leaf
            var secondChild = context.GetChild(1) as Describe09Parser.ProducerContext;
            if (secondChild == null)
            {
                throw new ArgumentException(
            ...
            else if (nextChild is Describe09Parser.ItemContext)
            {
                var ctxt = nextChild as Describe09Parser.ItemContext;
            ...
            else if (nextChild is Describe09Parser.ExpressionContext)
            {
                var ctxt = nextChild as Describe09Parser.ExpressionContext;
            ...
            else if (nextChild is Describe09Parser.Item_or_expression_listContext)
            {
                var ctxt = nextChild as Describe09Parser.Item_or_expression_listContext;
        ...
        /// </summary>
        private List<AstExpressionLineNode> DoItemOrExpressionList(Describe09Parser.Item_or_expression_listContext context)
        {
            int childCount = context.ChildCount;
            List<AstExpressionLineNode> lines = new List<AstExpressionLineNode>();
            ...
            {
                var item = DoItem((lastChild as Describe09Parser.ItemContext)!);
                var line = AstFactory.CreateExpressionLineNode(item, null);
                lines.Add(line);
            }
            else if (lastChild is Describe09Parser.ExpressionContext)
            {
                var expression = DoExpression((lastChild as Describe09Parser.ExpressionContext)!);
        ...
        /// "item_or_expression_part : expression (SEPARATOR)? ;"
        /// </summary>
        private AstExpressionLineNode DoItemOrExpressionPart(Describe09Parser.Item_or_expression_partContext context)
        {
            int childCount = context.ChildCount;
            var firstChild = context.GetChild(0);

            if(firstChild is  Describe09Parser.ItemContext)
            {
                var item = DoItem((firstChild as Describe09Parser.ItemContext)!);
            ...
            else if(firstChild is Describe09Parser.ExpressionContext)
            {
                var expression = DoExpression((firstChild as Describe09Parser.ExpressionContext)!);
        ...
        /// </summary>
        private AstItemNode DoItem(Describe09Parser.ItemContext context)
        {
            int childCount = context.ChildCount;
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                else if (cur is Describe09Parser.Text_chunkContext)
                {
                    var child = (cur as Describe09Parser.Text_chunkContext)!;
            ...
            AstItemNode item = AstFactory.CreateItemNode(infochunk, tag, links, decorators);
            return item;
        }


        // do ProductionArrow Leaf
        private AstLeafNode doProductionArrow(Describe09Parser.ProducerContext context)
        {
            int childCount = context.ChildCount;
        ...
        private AstDecoratorNode doDecorator(ITerminalNode token)
        {
            ...
            {
                AstLeafNode name = doLeaf(AstLeafType.Text, token, oindex + 1, sep[0].Length);
                AstLeafNode value = doLeaf(AstLeafType.Text, token, oindex + 1 + sep[0].Length, sep[1].Length);
                AstDecoratorNode decorator = AstFactory.CreateDoubleDecoratorNode(o, name, value, c);
                return decorator;
            }
            else if (sep.Length == 3)
            {
                AstLeafNode category = doLeaf(AstLeafType.Text, token, oindex + 1, sep[0].Length);
                AstLeafNode name = doLeaf(AstLeafType.Text, token, oindex + 1 + sep[0].Length, sep[1].Length);
                AstLeafNode value = doLeaf(AstLeafType.Text, token, oindex + 1 + sep[0].Length + sep[1].Length, sep[2].Length);
            ...
            else if (sep.Length > 3)
            {
                ...
                AstLeafNode category = doLeaf(AstLeafType.Text, token, oindex + 1, sep[0].Length);
                AstLeafNode name = doLeaf(AstLeafType.Text, token, oindex + 1 + sep[0].Length, sep[1].Length);
                AstLeafNode value = doLeaf(AstLeafType.Text, token, oindex + 1 + sep[0].Length + sep[1].Length, x);
                AstDecoratorNode decorator = AstFactory.CreateTripleDecoratorNode(o, category, name, value, c);
                return decorator;
            }
            else
