...
{
    /// <summary>
    /// An Ast visitor for Describe Language version 0.7
    ...
    public class AstVisitor07
    {
        ...
        public AstScriptureNode TranslateScripture(Describe07Parser.ScriptureContext context, string filename = "")
        {
            ...
            {
                AstExpressionNode expr = DoExpression((child as Describe07Parser.ExpressionContext)!);
                expressions.Add(expr);
            }
            else if (child is Describe07Parser.Expression_listContext)
            {
                expressions = DoExpressionList((child as Describe07Parser.Expression_listContext)!);
            }
            else
            {
        ...
        /// </summary>
        private List<AstExpressionNode> DoExpressionList(Describe07Parser.Expression_listContext context)
        {
        ...
        /// </summary>
        private AstExpressionNode DoExpression(Describe07Parser.ExpressionContext context)
        {
            // sanity check
            int childCount = context.ChildCount;
            ...
            // get arrow leaf
            var secondChild = context.GetChild(1) as Describe07Parser.ProducerContext;
            if (secondChild == null)
            {
                throw new ArgumentException(
            ...
            else if (nextChild is Describe07Parser.ItemContext)
            {
                var ctxt = nextChild as Describe07Parser.ItemContext;
            ...
            else if (nextChild is Describe07Parser.ExpressionContext)
            {
                var ctxt = nextChild as Describe07Parser.ExpressionContext;
            ...
            else if (nextChild is Describe07Parser.Item_or_expression_listContext)
            {
                var ctxt = nextChild as Describe07Parser.Item_or_expression_listContext;
        ...
        /// </summary>
        private List<AstExpressionLineNode> DoItemOrExpressionList(Describe07Parser.Item_or_expression_listContext context)
        {
            int childCount = context.ChildCount;
            List<AstExpressionLineNode> lines = new List<AstExpressionLineNode>();
            ...
            {
                var item = DoItem((lastChild as Describe07Parser.ItemContext)!);
                var line = AstFactory.CreateExpressionLineNode(item, null);
                lines.Add(line);
            }
            else if (lastChild is Describe07Parser.ExpressionContext)
            {
                var expression = DoExpression((lastChild as Describe07Parser.ExpressionContext)!);
        ...
        /// "item_or_expression_part : expression (SEPARATOR)? ;"
        /// </summary>
        private AstExpressionLineNode DoItemOrExpressionPart(Describe07Parser.Item_or_expression_partContext context)
        {
            int childCount = context.ChildCount;
            var firstChild = context.GetChild(0);

            if(firstChild is  Describe07Parser.ItemContext)
            {
                var item = DoItem((firstChild as Describe07Parser.ItemContext)!);
            ...
            else if(firstChild is Describe07Parser.ExpressionContext)
            {
                var expression = DoExpression((firstChild as Describe07Parser.ExpressionContext)!);
        ...
        /// "item : (text_chunk)+ TAG? ;"
        /// </summary>
        private AstItemNode DoItem(Describe07Parser.ItemContext context)
        {
            int childCount = context.ChildCount;
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                else if (cur is Describe07Parser.Text_chunkContext)
                {
                    var child = (cur as Describe07Parser.Text_chunkContext)!;
                ...
                else if(cur is ITerminalNode)
                {
                    ...
                    if (GetTokenType(token) != "TAG") continue;
                    tag = doTag(token);
                }
                else
                {
            ...
            // get position
            var firstChild = context.GetChild(0) as Describe07Parser.Text_chunkContext;
            ITerminalNode? firstToken = firstChild?.GetChild(0) as ITerminalNode;
            ITerminalNode? lastToken = null;
            var lastChild = context.GetChild(childCount - 1);
            if (lastChild is ITerminalNode) lastToken = lastChild as ITerminalNode;
            else lastToken = lastChild.GetChild(0) as ITerminalNode;
            ...
            AstItemNode item = AstFactory.CreateItemNode(infochunk, tag);
            return item;
        }


        // do ProductionArrow Leaf
        private AstLeafNode doProductionArrow(Describe07Parser.ProducerContext context)
        {
            int childCount = context.ChildCount;
