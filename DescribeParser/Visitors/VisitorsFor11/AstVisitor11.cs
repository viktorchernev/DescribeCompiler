...
{
    /// <summary>
    /// An Ast visitor for Describe Language version 1.1
    /// Used to build Abstract Syntax Trees.
    /// </summary>
    public class AstVisitor11
    {
        /// <summary>
        /// Ctor.
        /// </summary>
        ...
        public AstScriptureNode TranslateScripture(Describe11Parser.ScriptureContext context, string filename = "")
        {
            ...
            {
                AstExpressionNode expr = DoExpression((child as Describe11Parser.ExpressionContext)!);
                expressions.Add(expr);
            }
            else if (child is Describe11Parser.Expression_listContext)
            {
                expressions = DoExpressionList((child as Describe11Parser.Expression_listContext)!);
            }
            else
            {
        ...
        /// </summary>
        private List<AstExpressionNode> DoExpressionList(Describe11Parser.Expression_listContext context)
        {
        ...
        /// </summary>
        private AstExpressionNode DoExpression(Describe11Parser.ExpressionContext context)
        {
            // sanity check
            int childCount = context.ChildCount;
            ...
            // get arrow leaf
            var secondChild = context.GetChild(1) as Describe11Parser.ProducerContext;
            if (secondChild == null)
            {
                throw new ArgumentException(
            ...
            else if (nextChild is Describe11Parser.ItemContext)
            {
                var ctxt = nextChild as Describe11Parser.ItemContext;
            ...
            else if (nextChild is Describe11Parser.ExpressionContext)
            {
                var ctxt = nextChild as Describe11Parser.ExpressionContext;
            ...
            else if (nextChild is Describe11Parser.Item_or_expression_listContext)
            {
                var ctxt = nextChild as Describe11Parser.Item_or_expression_listContext;
        ...
        /// </summary>
        private List<AstExpressionLineNode> DoItemOrExpressionList(Describe11Parser.Item_or_expression_listContext context)
        {
            int childCount = context.ChildCount;
            List<AstExpressionLineNode> lines = new List<AstExpressionLineNode>();
            ...
            {
                var item = DoItem((lastChild as Describe11Parser.ItemContext)!);
                var line = AstFactory.CreateExpressionLineNode(item, null);
                lines.Add(line);
            }
            else if (lastChild is Describe11Parser.ExpressionContext)
            {
                var expression = DoExpression((lastChild as Describe11Parser.ExpressionContext)!);
        ...
        /// "item_or_expression_part : expression (SEPARATOR)? ;"
        /// </summary>
        private AstExpressionLineNode DoItemOrExpressionPart(Describe11Parser.Item_or_expression_partContext context)
        {
            int childCount = context.ChildCount;
            var firstChild = context.GetChild(0);

            if(firstChild is Describe11Parser.ItemContext)
            {
                var item = DoItem((firstChild as Describe11Parser.ItemContext)!);
            ...
            else if(firstChild is Describe11Parser.ExpressionContext)
            {
                var expression = DoExpression((firstChild as Describe11Parser.ExpressionContext)!);
        ...
        /// <summary>
        /// "item : TILDE? (text_chunk)+ (LINK)+"
        /// "item : TILDE? (text_chunk)+ (DECORATOR)+"
        /// "item : TILDE? (text_chunk)+ (LINK)+ TAG"
        /// "item : TILDE? (text_chunk)+ TAG (LINK)+ "
        /// "item : TILDE? (text_chunk)+ (DECORATOR)+ TAG "
        /// "item : TILDE? (text_chunk)+ TAG(DECORATOR)+ "
        /// "item : TILDE? (text_chunk)+ (DECORATOR)+ (LINK)+ "
        /// "item : TILDE? (text_chunk)+ (LINK)+ (DECORATOR)+ "
        /// "item : TILDE? (text_chunk)+ TAG(DECORATOR)+ (LINK)+ "
        /// "item : TILDE? (text_chunk)+ (DECORATOR)+ TAG(LINK)+ "
        /// "item : TILDE? (text_chunk)+ (DECORATOR)+ (LINK)+ TAG "
        /// "item : TILDE? (text_chunk)+ TAG(LINK)+ (DECORATOR)+ "
        /// "item : TILDE? (text_chunk)+ (LINK)+ TAG(DECORATOR)+ "
        /// "item : TILDE? (text_chunk)+ (LINK)+ (DECORATOR)+ TAG ; "
        /// </summary>
        private AstItemNode DoItem(Describe11Parser.ItemContext context)
        {
            int childCount = context.ChildCount;
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                else if (cur is Describe11Parser.Text_chunkContext)
                {
                    var child = (cur as Describe11Parser.Text_chunkContext)!;
                ...
                else if(cur is ITerminalNode)
                {
                    ...
                    else if (GetTokenType(token) == "TAG")
                    {
                        tag = doTag(token);
                    }
                    ...
                    else if (GetTokenType(token) == "DECORATOR")
                    {
                        AstDecoratorNode decorator = doDecorator(token);
        ...
        private AstLeafNode doProductionArrow(Describe11Parser.ProducerContext context)
        {
            int childCount = context.ChildCount;
        ...
        // do Leaf
        private AstDecoratorNode doDecorator(ITerminalNode token)
        {
            ...
            {
                throw new ArgumentException("Invalid input string format in Decorator.");
            }

            AstLeafNode o = doLeaf(AstLeafType.OpenTag, token, 0, oindex + 2);
            AstLeafNode c = doLeaf(AstLeafType.CloseTag, token, cindex, text.Length - cindex);
            string inside = text.Substring(oindex + 2, cindex - oindex - 2);
            string[] sep = inside.Split('|');

            if (sep.Length == 1)
            {
                AstLeafNode name = doLeaf(AstLeafType.Text, token, oindex + 2, cindex - oindex - 2);
                AstDecoratorNode decorator = AstFactory.CreateSimpleDecoratorNode(o, name, c);
                return decorator;
            }
            else if (sep.Length == 2)
            {
                AstLeafNode name = doLeaf(AstLeafType.Text, token, oindex + 2, sep[0].Length);
                AstLeafNode value = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length, sep[1].Length);
                AstDecoratorNode decorator = AstFactory.CreateDoubleDecoratorNode(o, name, value, c);
                return decorator;
            }
            else if (sep.Length == 3)
            {
                AstLeafNode category = doLeaf(AstLeafType.Text, token, oindex + 2, sep[0].Length);
                AstLeafNode name = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length, sep[1].Length);
                AstLeafNode value = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length + sep[1].Length, sep[2].Length);
            ...
            else if (sep.Length > 3)
            {
                ...
                AstLeafNode category = doLeaf(AstLeafType.Text, token, oindex + 2, sep[0].Length);
                AstLeafNode name = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length, sep[1].Length);
                AstLeafNode value = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length + sep[1].Length, x);
                AstDecoratorNode decorator = AstFactory.CreateTripleDecoratorNode(o, category, name, value, c);
                return decorator;
            }
            else
        ...
        private AstLinkNode doLink(ITerminalNode token)
        {
            ...
            if (sep.Length == 1)
            {
                url = doLeaf(AstLeafType.Text, token, oindex + 2, cindex - oindex - 2);
            }
            else if (sep.Length == 2)
            {
                url = doLeaf(AstLeafType.Text, token, oindex + 2, sep[0].Length);
                title = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length, sep[1].Length);
            }
            else if (sep.Length == 3)
            {
                url = doLeaf(AstLeafType.Text, token, oindex + 2, sep[0].Length);
                title = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length, sep[1].Length);
                letter = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length + sep[1].Length, sep[2].Length);
            }
            else if (sep.Length > 3)
            {
                int x = sep[2].Length;
                ...
                url = doLeaf(AstLeafType.Text, token, oindex + 2, sep[0].Length);
                title = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length, sep[1].Length);
                letter = doLeaf(AstLeafType.Text, token, oindex + 2 + sep[0].Length + sep[1].Length, x);
            }
            else
            {
                throw new ArgumentException(
        ...
        private AstTagNode doTag(ITerminalNode token)
        {
            ...
            AstLeafNode o = doLeaf(AstLeafType.OpenTag, token, 0, oindex + 2);
            AstLeafNode d = doLeaf(AstLeafType.Text, token, oindex + 2, cindex - oindex - 2);
            AstLeafNode c = doLeaf(AstLeafType.CloseTag, token, cindex, text.Length - cindex);
