...
exclude: true
---
Describe version 1.1, code-named Doubles is the second official version of the language, although it is similar to a variant of version 1.0. The difference is that, in this version, the `<<` tags `>>`, `[[` links `]]` and `{{` decorators `}}` are enclosed in double brackets, thus allowing for text that contains single brackets of these kinds be conveniently written, without the need to escape said brackets.
