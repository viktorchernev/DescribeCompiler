...
namespace DescribeParser.Unfold
{
    public partial class DescribeUnfoldV3
    {
        ...
        private string AllFiles_ToString()
        {
            ...
            foreach(KeyValuePair<int, string> kvp in AllFiles)
            {
                text += INDENT + INDENT + '"' + kvp.Key.ToString() + '"'; 
                text += " \"" + kvp.Value + '"' + Environment.NewLine;
            }
        ...
        //ToString() - Main Data
        private string Ids_ToString()
        {
            string text = INDENT + ".Ids" + Environment.NewLine;
        ...
        private string Productions_ToString()
        {
            ...
            foreach (KeyValuePair<int, List<int>> kvp in Productions)
            {
        ...
        private string Tildes_ToString()
        {
            ...
            foreach (KeyValuePair<int, List<int>> kvp in Tildes)
            {
                text += INDENT + INDENT + '"' + kvp.Key + "\" -> ";
                if (kvp.Value.Count > 10)
                {
        ...
        private string Links_ToString()
        {
            ...
            foreach (KeyValuePair<int, List<DescribeLink>> kvp in Links)
            {
        ...
        private string Decorators_ToString()
        {
            ...
            foreach (KeyValuePair<int, List<DescribeDecorator>> kvp in Decorators)
            {
                if (kvp.Value.Count == 0)
                {
                    text += INDENT + INDENT + '"' + kvp.Key + "\" - ";
        ...
        private string ProdidFile_ToString()
        {
            ...
            foreach (KeyValuePair<int, List<int>> kvp in ProdidFile)
            {
                text += INDENT + INDENT + '"' + kvp.Key + "\" - \"" + kvp.Value[0] + "\"" + Environment.NewLine;
                for(int i = 1; i < kvp.Value.Count; i++)
                {
                    text += INDENT + INDENT + '"' + kvp.Key + "\" - \"" + kvp.Value[i] + "\"" + Environment.NewLine;
                }
            }
            text += Environment.NewLine;
            return text;
        ...
        private string ItemidFile_ToString()
        {
            ...
            foreach (KeyValuePair<int, List<int>> kvp in ItemidFile)
            {
                text += INDENT + INDENT + '"' + kvp.Key + "\" - \"" + kvp.Value[0] + "\"" + Environment.NewLine;
                for (int i = 1; i < kvp.Value.Count; i++)
                {
